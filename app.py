...
for key, default in [
    ...
    ('opt_results', None), ('opt_meta', None),
]:
...
if st.session_state.fwd_data is not None and loc_file is not None:
    ...
    if st.button("🏁 Spustit optimalizaci", type="primary"):
        ...
        res['KGJ starty'] = [val(start, t) for t in range(T)] if use_kgj else [0] * T
        res['KGJ_on'] = [val(on, t) for t in range(T)] if use_kgj else [0] * T
        ...
        _kgj_hours = sum(1 for t in range(T) if val(on, t) > 0.5) if use_kgj else 0
        _boil_eff  = p.get('boil_eff', 0.95) or 0.95
        st.session_state.opt_results = res.copy()
        st.session_state.opt_meta = {
            ...
            'coverage':        100 * (1 - res['Shortfall [MW]'].sum() / max((res['Poptávka tepla [MW]'] * p['h_cover']).sum(), 1e-9)),
            'total_ee_gen':    res['EE z KGJ [MW]'].sum() + res['EE z FVE [MW]'].sum(),
            'kgj_hours':       _kgj_hours,
            'T':               T,
            'p':               p.copy(),
            'use_kgj':         use_kgj,
            'use_boil':        use_boil,
            ...
            'use_bess':        use_bess,
            'use_fve':         use_fve,
            'use_ext_heat':    use_ext_heat,
            'boil_eff':        _boil_eff,
            'df_imp_price':    df['imp_price'].values.tolist(),
        }

    # ════════════════════════════════════════════════════
    # ZOBRAZENÍ VÝSLEDKŮ (přetrvává přes tab přepnutí)
    # ════════════════════════════════════════════════════
    if st.session_state.opt_results is not None:
        ...
        T            = meta['T']
        p            = meta['p']
        use_kgj      = meta['use_kgj']
        use_boil     = meta['use_boil']
        use_ek       = meta['use_ek']
        use_tes      = meta['use_tes']
        use_bess     = meta['use_bess']
        use_fve      = meta['use_fve']
        use_ext_heat = meta['use_ext_heat']
        boil_eff     = meta['boil_eff']
        total_profit    = meta['total_profit']
        total_shortfall = meta['total_shortfall']
        coverage        = meta['coverage']
        total_ee_gen    = meta['total_ee_gen']
        kgj_hours       = meta['kgj_hours']

        st.subheader("📋 Výsledky optimalizace")
        st.write(f"**Solver status:** {meta['status_str']} | **Účelová funkce:** {meta['obj_val']:,.0f} €")

        # ── Metriky ───────────────────────────────────────
        # (proměnné total_profit, total_shortfall atd. jsou načteny výše z meta)
        ...
        c_gas_kgj = sum(
            (p.get('kgj_gas_fix_price', res['Cena plyn [€/MWh]'].iloc[t]) + p['gas_dist'])
            * res['KGJ [MW_th]'].iloc[t] / p['k_eff_th']
            for t in range(T)
        ) if use_kgj else 0
        c_gas_boil = sum(
            (p.get('boil_gas_fix_price', res['Cena plyn [€/MWh]'].iloc[t]) + p['gas_dist'])
            * res['Kotel [MW_th]'].iloc[t] / boil_eff
            for t in range(T)
        ) if use_boil else 0
        c_ee_imp   = ((res['Cena EE [€/MWh]'] + p['dist_ee_buy']) * res['EE import [MW]']).sum()
        c_imp_heat = (res['Cena importu tepla [€/MWh]'] * res['Import tepla [MW_th]']).sum() if use_ext_heat else 0
        c_starts   = p['k_start_cost'] * res['KGJ starty'].sum() if use_kgj else 0
                    ...
                    for ci, cn in enumerate(df_exp.columns):
                        ...
                        if cn == 'Čas':
                            ...
                            if math.isnan(num) or math.isinf(num):
                                num = 0.0
                            if cn in money_c:
                                ws.write_number(ri + 1, ci, num, fmt_num0)
                            else:
                                ws.write_number(ri + 1, ci, num, fmt_num2)
